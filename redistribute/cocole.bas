...
10HCLS0:HCOLOR5:HPRINT(0,0),"COCOdle":R=RND(-TIMER):OPEN"R",#1,"GUESS.TXT",5:FIELD#1,5AS R$:N=LOF(1):R=RND(N):GET#1,R:W$=R$:CLOSE1:HCOLOR4:HPRINT(6,2),"ABCDEFGHIJKLMNOPQRSTUVWXYZ":HCOLOR5:HCOLOR1:FORY=7TO17STEP2
...
64NEXTJ:X=I*2+13:Y=G*2+5:HCOLORC:HLINE(X*8-2,Y*8-2)-(X*8+8,Y*8+8),PSET,BF:HCOLOR0:HPRINT(X,Y),C1$:IFC=4 THENPOKE&HF015,&H21:HPRINT(6+ASC(C1$)-ASC("A"),2)," ":POKE&HF015,&HAA ELSEHCOLOR1:HPRINT(6+ASC(C1$)-ASC("A"),2),C1$
65NEXTI:HCOLOR5:IFG$=W$ THENM$="You won in"+STR$(G)+" guesses!":HPRINT(10,21),M$:GOSUB5000:GOTO10
