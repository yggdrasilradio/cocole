...
10HCLS0:HCOLOR1:HPRINT(0,0),"COCOdle":HCOLOR5:HPRINT(27,0),"by Rick Adams":R=RND(-TIMER):POKE&HFFD8,0:OPEN"R",#1,"GUESS.TXT",5:FIELD#1,5AS R$:N=LOF(1):R=RND(N):GET#1,R:W$=R$:CLOSE1:POKE&HFFD9,0:HCOLOR4
11HPRINT(6,3),"ABCDEFGHIJKLMNOPQRSTUVWXYZ":HCOLOR1:FORY=7TO17STEP2:Y8=Y*8:FORX=15TO23STEP2:X8=X*8:HLINE(X8-3,Y8-3)-(X8+9,Y8+9),PSET,B:NEXTX:NEXTY:G=1:HCOLOR5
60GOSUB4000:IFLEN(G$)<>5 THENM$="Invalid" ELSEGOSUB2000
61IFM$<>"" THENSOUND1,2:GOTO60
62G2$=G$:S$=W$:R$="44444":FORI=1TO5:C1$=MID$(G$,I,1):C2$=MID$(S$,I,1):IFC1$=C2$ THENMID$(R$,I,1)="2":MID$(G$,I,1)=".":MID$(S$,I,1)=" "
63NEXTI:FORI=1TO5:C1$=MID$(G$,I,1):FORJ=1TO5:C2$=MID$(S$,J,1):IFC1$=C2$ THENMID$(R$,I,1)="3":MID$(G$,I,1)=".":MID$(S$,J,1)=" "
64NEXTJ:NEXTI
90FORI=1TO5:C$=MID$(G2$,I,1):C=VAL(MID$(R$,I,1)):X=I*2+13:Y=G*2+5:HCOLORC:HLINE(X*8-2,Y*8-2)-(X*8+8,Y*8+8),PSET,BF:HCOLOR0:HPRINT(X,Y),C$:POKE&HF015,&H21
95IFINSTR(W$,C$)>0 THENHCOLOR1:HPRINT(6+ASC(C$)-ASC("A"),3),C$ ELSEHPRINT(6+ASC(C$)-ASC("A"),3)," "
96POKE&HF015,&HAA:NEXTI:HCOLOR5:IFG2$=W$ THENM$="You won in"+STR$(G)+" guesses!":HPRINT(10,21),M$:GOSUB5000:GOTO10
