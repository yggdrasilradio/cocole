...
10HCLS0:HCOLOR1:HPRINT(0,0),"COCOdle":HCOLOR5:HPRINT(27,0),"by Rick Adams":R=RND(-TIMER):OPEN"R",#1,"GUESS.TXT",5:FIELD#1,5AS R$:N=LOF(1):R=RND(N):GET#1,R:W$=R$:CLOSE1:W$="ROUTE":HCOLOR4:HPRINT(6,3),"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
64NEXTJ:X=I*2+13:Y=G*2+5:HCOLORC:HLINE(X*8-2,Y*8-2)-(X*8+8,Y*8+8),PSET,BF:HCOLOR0:HPRINT(X,Y),C$:POKE&HF015,&H21:IFC=4 THENHPRINT(6+ASC(C1$)-ASC("A"),3)," " ELSEHCOLOR1:HPRINT(6+ASC(C1$)-ASC("A"),3),C1$
